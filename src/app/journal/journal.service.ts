...
import { Observable, BehaviorSubject, combineLatest, from, empty } from "rxjs";
...
export class JournalService extends EventEmitter {
  ...
  private _currentSystemStarPos = new BehaviorSubject<[number, number, number]>(
    [NaN, NaN, NaN]
  );
  ...
  constructor(
    ...
    this.journalDB
      .getEntry<{ key: string; value: string }>("currentState", "currentSystem")
      .then((currentSystem) => {
        if (
          currentSystem &&
          currentSystem.value &&
          typeof currentSystem.value === "string"
        ) {
    ...
    this.journalDB
      .getEntry<{ key: string; value: string }>(
        "currentState",
        "currentStation"
      )
      .then((currentStation) => {
        if (
          ...
          typeof currentStation.value === "string"
        ) {
    ...
    this.journalDB
      .getEntry<{ key: string; value: number }>(
        "currentState",
      ...
      .then((currentSystemAddress) => {
        if (
          currentSystemAddress &&
          currentSystemAddress.value &&
          typeof currentSystemAddress.value === "number"
        ) {
    ...
    this.journalDB
      .getEntry<{ key: string; value: [number, number, number] }>(
        "currentState",
        "currentSystemStarPos"
      )
      .then((currentSystemStarPos) => {
        if (
          currentSystemStarPos &&
          currentSystemStarPos.value &&
          currentSystemStarPos instanceof Array
        ) {
    ...
    this.journalDB
      .getEntry<{ key: string; value: string }>("currentState", "cmdrName")
      .then((cmdrName) => {
    ...
    this.journalDB
      .getEntry<{ key: string; value: string }>("currentState", "shipID")
      .then((shipID) => {
    ...
    localStorage["logDir"] = this._logDir =
      localStorage["logDir"] || this.detectDir() || this.selectDirDialog();
  ...
  private async streamAll(dir: string) {
    ...
    fs.readdir(dir, async (err, files) => {
      ...
      let journalFilePaths = files
        .filter((path: string): boolean => {
          return this.re.logfile.test(path);
        })
        .sort();
      if (journalFilePaths.length === 0) {
        return;
      }
      ...
      for (let i = 0; i < journalFilePaths.length - 1; i++) {
        ...
        await this.journalDB
          .getEntry("completedJournalFiles", path)
          .then((data) => {
            if (!data) {
                  ...
                  .on("data", (data: journal.JournalEventsUnion) => {
                    ...
                    this.handleEvent(data)
                      ...
                      .catch((originalError) => {
                        this.logger.error({
                          originalError,
                          message: "Error in initial stream",
                        });
                        stream.resume();
                      });
                  })
                  .on("end", () => {
                    this.journalDB.addEntry("completedJournalFiles", {
                      filename: path,
                    });
                    this._directoryReadProgress.next(
                      ((i + 1) / (journalFilePaths.length - 1)) * 100
                    );
                    resolve();
                  });
              });
      ...
      Promise.all(journalPromises)
        .then(() => {
          this.tailStream(`${dir}/${this.currentLogFile}`, {
            start: this.offset,
            encoding: "utf8",
          });
        })
        .catch((originalError) =>
          this.logger.error({
            originalError,
            message: "Initial journal stream error",
          })
        );
    });
  }
  ...
  private tailStream(
    path: string,
    options: { start: number; encoding: string }
  ) {
    //avoid situation where logfile changes before async operations complete
    this.journalDB
      .getEntry("completedJournalFiles", this.currentLogFile)
      .then((data) => {
        if (!data) {
          this.journalDB.addEntry("completedJournalFiles", {
    ...
    newStream
        ...
        let eventHandler = this.handleEvent(data)
          ...
          .catch((reason: string) => {
            this.logger.error({
              ...
              message: "handleEvent rejected in tailStream",
            });
            newStream.resume();
          });
      ...
      .on("error", (originalError: any) =>
        this.logger.error({ originalError, message: "tailstream error" })
      )
  ...
  public watchLogDir(): void {
    ...
    new Promise((resolve, reject) => {
      ...
      watcher.on("change", async (event: string, eventPath: string) => {
        ...
        //TODO: Add handling for new file types emitted by game
          ...
          const marketData = readJSONFile<MarketJSON>(
            `${this._logDir}/${eventPath}`
          );

          combineLatest(this.cmdrName, from(marketData))
            .pipe(
              take(1),
              catchError((err) => {
                this.logger.error(err);
            ...
            .subscribe(([cmdrName, marketData]) => {
              this.eddn.sendCommodityEvent(marketData, cmdrName);
            });
        }
      });
    });
  ...
  private async handleEvent(data: journal.JournalEventsUnion): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.cmdrUsernameMatch().subscribe(
        (match) => {
          if (!match && data.event !== journal.JournalEvents.loadGame) {
            return reject("CMDR/Username Mismatch");
          }
          if (this._beta && data.event !== journal.JournalEvents.fileHeader) {
            return reject("Ignoring BETA event");
          }
          ...
          switch (data.event) {
            ...
            case journal.JournalEvents.missionCompleted: {
              if (!this.firstStream) {
                let missionCompleted: MissionCompleted = Object.assign(
                ...
                if (missionCompleted.MaterialsReward) {
                  for (let material of missionCompleted.MaterialsReward) {
                    this.journalDB
                      .getEntry("materials", material.Name.toLowerCase())
                      .then((existingMaterial: Material) => {
                        let updatedMaterial: Material;
                        if (existingMaterial) {
                          updatedMaterial = Object.assign(
                            existingMaterial,
                            material,
                            { Count: existingMaterial.Count + material.Count }
                          );
                        } else {
                        ...
                        return this.journalDB
                          .putEntry("materials", updatedMaterial)
                          .then(() => {
            ...
            case journal.JournalEvents.missionAccepted: {
              this.journalDB
                .getEntry(
                  journal.JournalEvents.missionAccepted,
                  (<journal.MissionAccepted>data).MissionID
                )
                .then((result) => {
                  if (!result) {
                    this.journalDB
                      .addEntry(journal.JournalEvents.missionAccepted, data)
                      .then(() => {
                        resolve(data);
                      })
                      .catch((err) => {
            ...
            case journal.JournalEvents.loadGame: {
              let loadGame: journal.LoadGame = Object.assign(
                new journal.LoadGame(),
                data
              );
              this.ngZone.run(() => this._cmdrName.next(loadGame.Commander));
              this.journalDB
                .putCurrentState({ key: "cmdrName", value: loadGame.Commander })
                .then(() => resolve(data))
                .catch((originalError) => {
                  this.logger.error({
                    originalError,
                    message: "handleEvent failure",
                  });
                  resolve(data);
                });
              break;
            ...
            case journal.JournalEvents.newCommander: {
              ...
              this.journalDB
                .putCurrentState({ key: "cmdrName", value: data.Name })
                .then(() => resolve(data))
                .catch((originalError) => {
                  this.logger.error({
                    originalError,
            ...
            case journal.JournalEvents.docked: {
              ...
              let docked: journal.Docked = Object.assign(
                new journal.Docked(),
                data
              );
              ...
              promises.push(
                this.journalDB
                  .putCurrentState({
                    key: "currentSystemAddress",
                    value: docked.SystemAddress,
                  })
                  .catch((originalError) =>
                    this.logger.error({
                      originalError,
              ...
              this.ngZone.run(() =>
                this._currentStation.next(docked.StationName)
              );
              this.ngZone.run(() =>
                this._currentSystem.next(docked.StarSystem)
              );
              this.ngZone.run(() =>
                this._currentSystemAddress.next(docked.SystemAddress)
              );
              ...
              if (!this.firstStream && !this.beta) {
                ...
                combineLatest(
                  ...
                  (cmdrName, starPos) => {
                    return { cmdrName, starPos };
                  }
                )
                  .pipe(take(1))
                  .subscribe((combined) => {
                    this.eddn.sendJournalEvent(
                      docked,
                      combined.cmdrName,
                      combined.starPos
                    );
            ...
            case journal.JournalEvents.undocked: {
              ...
              promises.push(
                this.journalDB
                  .putCurrentState({ key: "currentStation", value: null })
                  .catch((originalError) =>
                    this.logger.error({
                      originalError,
                      message: "handleEvent failure",
              ...
              Promise.all(promises)
                ...
                .catch((originalError) => {
                  this.logger.error({
                    originalError,
                    data,
                    message: "Undocked event failure",
                  });
                  resolve(data);
                });
            ...
            case journal.JournalEvents.location: {
              ...
              if (data.Docked && data.StationName) {
                ...
                this.ngZone.run(() =>
                  this._currentStation.next(data.StationName)
                );
              }
              this.ngZone.run(() => this._currentSystem.next(data.StarSystem));
              this.ngZone.run(() =>
                this._currentSystemStarPos.next(data.StarPos)
              );
              this.ngZone.run(() =>
                this._currentSystemAddress.next(data.SystemAddress)
              );

              if (!this.firstStream && !this.beta) {
                let location = Object.assign(new journal.Location(), data);
                this.cmdrName.pipe(take(1)).subscribe((cmdrName) => {
              ...
              Promise.all(promises)
                ...
                .catch((originalError) => {
                  this.logger.error({
                    originalError,
                    data,
                    message: "Location event Failure",
                  });
                  resolve(data);
                });
            ...
            case journal.JournalEvents.fsdJump: {
              let promises: Promise<any>[] = [];
              let fsdJump: journal.FSDJump = Object.assign(
                new journal.FSDJump(),
                data
              );
              ...
              this.ngZone.run(() =>
                this._currentSystemAddress.next(fsdJump.SystemAddress)
              );
              this.ngZone.run(() =>
                this._currentSystemStarPos.next(fsdJump.StarPos)
              );

              if (!this.firstStream && !this.beta) {
                this._cmdrName.pipe(take(1)).subscribe((cmdrName) => {
                ...
                combineLatest(
                  ...
                  .subscribe((combined) => {
                    this.eddn.sendJournalEvent(
                      scan,
                      combined.cmdrName,
                      combined.currentSystemStarPos,
                      combined.currentSystem,
                      combined.currentSystemAddress
                    );
                  });
              }
              resolve(data);
            ...
            case journal.JournalEvents.supercruiseEntry: {
              let supercruiseEntry: journal.SupercruiseEntry = Object.assign(
                new journal.SupercruiseEntry(),
                data
              );
              this.ngZone.run(() =>
              ...
              this.journalDB
                .putCurrentState({
                  key: "currentSystem",
                  value: supercruiseEntry.StarSystem,
                })
                .then(() => resolve(data))
                .catch((originalError) => {
                  this.logger.error({
            ...
            case journal.JournalEvents.supercruiseExit: {
              let supercruiseExit: journal.SupercruiseExit = Object.assign(
                new journal.SupercruiseExit(),
                data
              );
              this.ngZone.run(() =>
              ...
              this.journalDB
                .putCurrentState({
                  key: "currentSystem",
                  value: supercruiseExit.StarSystem,
                })
                .then(() => resolve(data))
                .catch((originalError) => {
                  this.logger.error({
            ...
            case journal.JournalEvents.loadout: {
              let promises: Promise<any>[] = [];
              let loadout: journal.Loadout = Object.assign(
                new journal.Loadout(),
                data
              );
              ...
              promises.push(
                this.journalDB
                  .putCurrentState({ key: "shipID", value: loadout.ShipID })
                  .catch((originalError) =>
                    this.logger.error({
                      originalError,
                      message: "handleEvent failure",
            ...
            case journal.JournalEvents.resurrect: {
              let resurrected: journal.Resurrect = Object.assign(
                new journal.Resurrect(),
                data
              );

              if (resurrected.Option !== "rebuy") {
                this._currentShipID.pipe(take(1)).subscribe((shipID) => {
                  let loadout: Loadout;
                  this.journalDB
                    .getEntry<Loadout>(DBStore.ships, shipID)
                    .then((ship: Loadout) => {
                      loadout = ship;
                    ...
                    .catch((originalError) => {
                      this.logger.error({
                        originalError,
                        data,
                        message: "Resurrected event failed to delete ship",
                      });
                      resolve(data);
                    });
            ...
            case journal.JournalEvents.shipyardSell: {
              let shipyardSell: journal.ShipyardSell = Object.assign(
                new journal.ShipyardSell(),
                data
              );

              this.journalDB
                .deleteEntry("ships", shipyardSell.SellShipID)
                .then(() => resolve(data))
                .catch((originalError) => {
                  this.logger.error({
                    originalError,
            ...
            case journal.JournalEvents.materials: {
              let materials: journal.Materials = Object.assign(
                new journal.Materials(),
                data
              );
                  ...
                  promises.push(
                    this.journalDB
                      ...
                      .then((existingMaterial) => {
                        let updatedMaterial = Object.assign(
                        ...
                        return this.journalDB
                          ...
                          .then(() =>
                            this.emit(
                              KOKJournalEvents.materialUpdate,
                              updatedMaterial
                            )
                          );
                      })
                      .catch((originalError) =>
                        this.logger.error({
                          originalError,
                          message: "Failed to write material",
                          data: material,
                        })
                      )
                  );
                ...
                materials.Raw.forEach((material) => {
                  promises.push(
                    this.journalDB
                      ...
                      .then((existingMaterial) => {
                        let updatedMaterial = Object.assign(
                          { Category: "$MICRORESOURCE_CATEGORY_Elements;" },
                          existingMaterial,
                          material
                        );
                        updatedMaterial.Name = updatedMaterial.Name.toLowerCase();
            ...
            case journal.JournalEvents.materialCollected: {
              if (!this.firstStream) {
                let material: MaterialCollected = Object.assign(
                  new MaterialCollected(),
                  data
                );
                this.journalDB
                  .getEntry<Material>("materials", material.Name.toLowerCase())
                  .then((existingMaterial) => {
                    let updatedMaterial: Material;
                    if (!existingMaterial) {
                      updatedMaterial = material;
            ...
            case journal.JournalEvents.materialDiscarded: {
              if (!this.firstStream) {
                let materialDiscarded: MaterialDiscarded = Object.assign(
                  new MaterialDiscarded(),
                  data
                );
                this.journalDB
                  .getEntry<Material>(
                    "materials",
                    materialDiscarded.Name.toLocaleLowerCase()
                  )
                  .then((existingMaterial) => {
                    let updatedMaterial: Material;
                    if (!existingMaterial) {
                      updatedMaterial = Object.assign({}, materialDiscarded, {
                        Count: existingMaterial.Count - materialDiscarded.Count,
                      });
                    } else {
                      updatedMaterial = Object.assign(
                        existingMaterial,
                        ...
                        {
                          Count:
                            existingMaterial.Count - materialDiscarded.Count,
                        }
                      );
                    }
                    updatedMaterial.Name = updatedMaterial.Name.toLowerCase();
            ...
            case journal.JournalEvents.materialTrade: {
              if (!this.firstStream) {
                let promises: Promise<any>[] = [];
                let materialTrade: MaterialTrade = Object.assign(
                  new MaterialTrade(),
                  data
                );
                ...
                promises.push(
                  this.journalDB
                    .getEntry<Material>(
                      "materials",
                      materialTrade.Received.Material.toLocaleLowerCase()
                    )
                    .then((existingMaterial) => {
                      let updatedMaterial: Material;
                      if (!existingMaterial) {
                        ...
                        updatedMaterial.Name_Localised =
                          materialTrade.Received.Material_Localised;
                        updatedMaterial.Category =
                          materialTrade.Received.Category;
                        updatedMaterial.Category_Localised =
                          materialTrade.Received.Category_Localised;
                        updatedMaterial.Count = materialTrade.Received.Quantity;
                      } else {
                        updatedMaterial = Object.assign(existingMaterial, {
                          Count:
                            existingMaterial.Count ||
                            0 + materialTrade.Received.Quantity,
                        });
                      }
                      updatedMaterial.Name = updatedMaterial.Name.toLowerCase();
                      return this.journalDB
                ...
                promises.push(
                  this.journalDB
                    .getEntry<Material>(
                      "materials",
                      materialTrade.Paid.Material.toLocaleLowerCase()
                    )
                    .then((existingMaterial) => {
                      let updatedMaterial = Object.assign(existingMaterial, {
                        Count:
                          existingMaterial.Count - materialTrade.Paid.Quantity,
                      });
                      updatedMaterial.Name = updatedMaterial.Name.toLowerCase();
                      return this.journalDB
                        .putEntry("materials", updatedMaterial)
            ...
            case journal.JournalEvents.engineerCraft: {
              if (!this.firstStream) {
                let engineerCraft: EngineerCraft = Object.assign(
                  new EngineerCraft(),
                  data
                );
                let promises: Promise<any>[] = [];
            ...
            case journal.JournalEvents.engineerContribution: {
              if (!this.firstStream) {
                let engineerContribution: EngineerContribution = Object.assign(
                  new EngineerContribution(),
                  data
                );
                if (engineerContribution.Material) {
                  this.journalDB
                    .getEntry<Material>(
                      "materials",
                      engineerContribution.Material.toLocaleLowerCase()
                    )
                    .then((existingMaterial) => {
                      let updatedMaterial = Object.assign(existingMaterial, {
                        Count:
                          existingMaterial.Count -
                          engineerContribution.Quantity,
                      });
                      updatedMaterial.Name = updatedMaterial.Name.toLowerCase();
                      return this.journalDB
                        .putEntry("materials", updatedMaterial)
            ...
            case journal.JournalEvents.scientificResearch: {
              if (!this.firstStream) {
                let scientificResearch: ScientificResearch = Object.assign(
                ...
                this.journalDB
                  .getEntry<Material>(
                    DBStore.materials,
                    scientificResearch.Name.toLowerCase()
                  )
                  .then((existingMaterial) => {
                    existingMaterial.Count -= scientificResearch.Count;
                    return this.journalDB
                      .putEntry(DBStore.materials, existingMaterial)
                      .then(() => {
                        this.emit(
                          KOKJournalEvents.materialUpdate,
                          existingMaterial
            ...
            case journal.JournalEvents.technologyBroker: {
              if (!this.firstStream) {
                let technologyBroker: TechnologyBroker = Object.assign(
                  new TechnologyBroker(),
                  data
                );
                let promises: Promise<any>[] = [];
