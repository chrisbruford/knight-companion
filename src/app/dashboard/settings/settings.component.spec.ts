...
describe("SettingsComponent", () => {
  ...
  beforeEach(async(() => {
    let _settingsService = jasmine.createSpyObj<SettingsService>(
      "SettingsService",
  ...
  beforeEach(() => {
    ...
    settingsService = de.injector.get(SettingsService) as jasmine.SpyObj<
      SettingsService
    >;
  ...
  describe("broadcasts", () => {
    it("should init in state from db", fakeAsync(() => {
      fixture.whenStable().then(() => {
        expect(component.settingsForm.get(AppSetting.broadcasts).value).toBe(
          true
        );
      });
    }));
    ...
    it("should toggle on/off", () => {
      settingsService.updateSetting.and.callFake(
        (setting: string, value: any) => Promise.resolve(true)
      );
      const broadcastsToggle: MatSlideToggle = de.query(
        By.css('[formControlName="broadcasts"] input')
      ).componentInstance;
      broadcastsToggle.change.emit(
        new MatSlideToggleChange(broadcastsToggle, true)
      );
      expect(settingsService.updateSetting).toHaveBeenCalledWith(
        AppSetting.broadcasts,
        jasmine.any(Boolean)
      );
    });
  });
  ...
  describe("inara", () => {
    it("should init in state from db", fakeAsync(() => {
      fixture.whenStable().then(() => {
        expect(
          component.settingsForm.get(AppSetting.inaraBroadcasts).value
        ).toBe(true);
        expect(component.settingsForm.get(AppSetting.inaraAPIKey).value).toBe(
          dummyAPIKey
        );
      });
    }));
    ...
    it("should toggle integration on/off", () => {
      ...
      const broadcastsToggle: MatSlideToggle = de.query(
        By.css('[formControlName="inaraBroadcasts"]')
      ).componentInstance;
      broadcastsToggle.change.emit(
        new MatSlideToggleChange(broadcastsToggle, true)
      );
      expect(settingsService.updateSetting).toHaveBeenCalledWith(
        AppSetting.inaraBroadcasts,
        jasmine.any(Boolean)
      );
    });
