...
describe("SettingsService",()=>{
    ...
    it('should pass settings to the DB service for persisting',()=>{
        settingsService.updateSetting('inaraAPIKey','123456789abcde');
        expect(dbServiceSpy.putEntry)
        .toHaveBeenCalledWith(
            DBStore.appSettings, 
            jasmine.objectContaining({key: 'inaraAPIKey', value: '123456789abcde'})
        )
    });
    ...
    it('should fetch settings from the DB service',(done)=>{
        ...
        let setting = settingsService.getSetting('inaraAPIKey');
