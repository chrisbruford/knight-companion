...
describe("Register component", () => {
  ...
  beforeEach(async(() => {
    ...
    fakeUserService.register.and.callFake(
      (user: NewUser): Observable<User> => of(null)
    );
    routerSpy = jasmine.createSpyObj<Router>("Router", ["navigateByUrl"]);
    snackBarSpy = jasmine.createSpyObj<MatSnackBar>("SnackBar", ["open"]);
    usernameValidatorSpy = jasmine.createSpyObj<UsernameValidator>(
      "UsernameValidator",
      ["checkUsername"]
    );
    usernameValidatorSpy.checkUsername.and.callFake(
      (control: AbstractControl) => of(null)
    );
    emailValidatorSpy = jasmine.createSpyObj<KOKEmailValidator>(
      "EmailValidator",
      ["checkEmail"]
    );
    emailValidatorSpy.checkEmail.and.callFake((control: AbstractControl) =>
      of(null)
    );
  ...
  it("should not submit an invalid form", () => {
    ...
    component.registrationForm.setValue({
      username:
        "thisusernameisridiculouslytoolongforustoeventhinkaboutusingimo.Letspretenditdidnthappen",
      password: "123456",
      ...
      gamerole: "Retreat",
      shipname:
        "thisshipnameisridiculouslytoolongforustoeventhinkaboutusingimo.Letspretenditdidnthappen",
  ...
  it("should be invalid if password confirmation doesn't match", () => {
    component.ngOnInit();
    component.registrationForm.setValue(
      Object.assign(fakeNewUserFormData, {
        confirmpassword: "this doesn't match"
      })
    );
    fixture.debugElement
      .query(By.css("form"))
  ...
  it("should be invalid if email confirmation doesn't match", () => {
    component.ngOnInit();
    component.registrationForm.setValue(
      Object.assign(fakeNewUserFormData, { confirmemail: "email@doesnt.match" })
    );
    fixture.debugElement
      .query(By.css("form"))
      .triggerEventHandler("ngSubmit", null);
  ...
  it("should not accept duplicate usernames", async(() => {
    component.ngOnInit();
    usernameValidatorSpy.checkUsername.and.returnValue(
      of({ usernameTaken: { value: "A Taken Username" } })
    );
    component.registrationForm.setValue(fakeNewUserFormData);
  ...
  it("should not accept duplicate email addresses", async(() => {
    component.ngOnInit();
    emailValidatorSpy.checkEmail.and.returnValue(
      of({ emailTaken: { value: "a@taken.email" } })
    );
    component.registrationForm.setValue(fakeNewUserFormData);
    fixture.whenStable().then(() => {
      expect(component.registrationForm.invalid).toBeTruthy();
